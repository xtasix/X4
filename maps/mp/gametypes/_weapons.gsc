...
//	thread maps\mp\_teargrenades::main();
	...
	level.Proximityc4DetectionGracePeriod = level.ex_Proximityc4DetectionGracePeriod;
...
watchGrenadeUsage()
{
	...
	for ( ;; )
	{
		...
		//CookBar Start
		self thread extreme\_ex_grenadecookbar::cookbar(weaponName);
...
burnplayer(eInflictor, eAttacker,sMeansOfDeath, sWeapon)
{
	if (self.isPlayer)
	{
		self.entity thread extreme\_ex_utils::napalmDamage(eAttacker);
	}
}
...
Proximityc4Detonation() 
{
	...
	damagearea = spawn("trigger_radius", self.origin + (0,0,0-level.Proximityc4DetonateRadius), 0, level.Proximityc4DetonateRadius, level.Proximityc4DetonateRadius*2);
	self thread deleteOnDeath( damagearea );
	...
	wait level.Proximityc4DetectionGracePeriod;
