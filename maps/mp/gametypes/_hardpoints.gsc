...
init()
{
	...
	precacheItem( "artillery_mp" );//needed for ambit arty but keep to add to later version!
	precacheItem( "planecrash_mp" );//needed for ambit plane crash and expo
	...
	if ( maps\mp\gametypes\_tweakables::getTweakableValue( "hardpoint", "allowsentrygun" ) )
	{
		level.hardpointItems["sentrygun_mp"] = priority;
		priority++;
	}	
	...
	if ( maps\mp\gametypes\_tweakables::getTweakableValue( "hardpoint", "allowstealth" ) )
	{
		level.hardpointItems["stealth_mp"] = priority;
		priority++;
	}
	
	if ( maps\mp\gametypes\_tweakables::getTweakableValue( "hardpoint", "allowac130" ) )
	{
	...
	level.hardpointHints["sentrygun_mp"] = &"MP_EARNED_SENTRYGUN";
	level.hardpointHints["helicopter_mp"] = &"MP_EARNED_HELICOPTER";
	level.hardpointHints["napalm_mp"] = &"MP_EARNED_NAPALM";
	level.hardpointHints["stealth_mp"] = &"MP_EARNED_STEALTH";
	level.hardpointHints["ac130_mp"] = &"MP_EARNED_AC130";
	level.hardpointHints["nuke_mp"] = &"MP_EARNED_NUKE"; 
	...
	level.hardpointHints["stealth_mp_not_available"] = &"EXTREME_STEALTH_NOT_AVAILABLE"; 
	level.hardpointHints["ac130_mp_not_available"] = &"EXTREME_AC130_NOT_AVAILABLE";
	level.hardpointHints["nuke_mp_not_available"] = &"EXTREME_NUKE_NOT_AVAILABLE";
	level.hardpointHints["sentrygun_mp_not_available"] = &"EXTREME_SENTRYGUN_NOT_AVAILABLE";
	...
	level.hardpointInforms["stealth_mp"] = "mp_killstreak_jet";
	level.hardpointInforms["ac130_mp"] = "ac130";
	level.hardpointInforms["nuke_mp"] = "mp_killstreak_nuke";
	level.hardpointInforms["sentrygun_mp"] = "mp_killstreak_sentrygun";
	
	//misc kill streak sounds
	level.hardpointInforms["napalm_mp"] = "jetstart";
	level.hardpointInforms["airstrike_mp"] = "jetstart";
	level.hardpointInforms["helicopter_mp"] = "heliaway";
	level.hardpointInforms["stealth_mp"] = "stealthbomber";
	level.hardpointInforms["ac130_mp"] = "ac130";
	level.hardpointInforms["nuke_mp"] = "nuclearstrike";

	maps\mp\gametypes\_rank::registerScoreInfo( "hardpoint", level.ex_score_hardpoint );
	...
	level.napalmfx2 = loadfx ("explosions/tanker_explosion");
	...
	game["dialog"]["ac130_friendly"] = "ac130_friendly";
	game["dialog"]["ac130_enemy"] = "ac130_enemy";
	...
	game["dialog"]["stealth_mp"] = "airstrike";
	game["dialog"]["ac130_mp"] = "airstrike";
	game["dialog"]["nuke_mp"] = "nuke_ready";
	game["dialog"]["sentrygun_mp"] = "sentrygun_ready";
	game["dialog"]["ac130_mp"] = "ac130_ready";
	...
	if(level.ex_helicopter_manned) maps\mp\_helicopter_manned::init();
	if(level.ex_helicopter_unmanned) maps\mp\_helicopter_unmanned::init();
	
	extreme\_ex_ac130::init();	
	extreme\_ex_uav::init();
	extreme\_ex_stealthbomber::init();
	extreme\_ex_nuke::init();
	extreme\_ex_sentrygun::init();
...
napalmburnThread()
{
	...
	for ( ; level.airstrikeDamagedEntsIndex < level.airstrikeDamagedEntsCount; level.airstrikeDamagedEntsIndex++ )
	{
		...
		if ( !ent.isPlayer || isAlive( ent.entity ) )
		{
			ent maps\mp\gametypes\_weapons::burnplayer(ent.eInflictor, ent.damageOwner, "MOD_PROJECTILE_SPLASH", "napalm_mp");
...
doPlaneStrike( owner, requiredDeathCount, bombsite, startPoint, endPoint, bombTime, flyTime, direction )
{
	...
	if( isDefined(owner.ex_hardpointtype) && owner.ex_hardpointtype == "airstrike_mp" ) plane setModel( "vehicle_mig29_desert" );
	else		
	if( isDefined(owner.ex_hardpointtype) && owner.ex_hardpointtype == "napalm_mp" ) plane setModel( "vehicle_mig29_desert" );
	
	plane.angles = direction;
	
	if( isDefined(owner.ex_hardpointtype) && owner.ex_hardpointtype == "airstrike_mp" ) plane thread playPlaneFx();

	else if( isDefined(owner.ex_hardpointtype) && owner.ex_hardpointtype == "napalm_mp" ) plane thread playPlaneFx();
...
callStrike_bombEffect( plane, launchTime, owner, requiredDeathCount )
{
	...
	if( isDefined(owner.ex_hardpointtype) && owner.ex_hardpointtype == "napalm_mp" ) playfxontag( level.napalmfx, newBomb, "tag_origin" );
	if( isDefined(owner.ex_hardpointtype) && owner.ex_hardpointtype == "napalm_mp" ) playfxontag( level.napalmfx2, newBomb, "tag_origin" );
	else
	...
	for( i = 0; i < repeat; i++ )
	{
		...
		if( isDefined(owner.ex_hardpointtype) && owner.ex_hardpointtype == "napalm_mp" )					  
		{
			thread losRadiusDamage( traceHit + (0,0,16), level.ex_napalm_radius, 0, 0, owner, bomb ); // targetpos, radius, maxdamage, mindamage, player causing damage, entity that player used to cause damage
		}
		else
		{
		...
		if ( i%3 == 0 )
		{
			if( isDefined(owner.ex_hardpointtype) && owner.ex_hardpointtype == "napalm_mp" ) thread playsoundinspace( "napalm_burst", traceHit );
...
{
	if(level.ex_kill_streaks) 
	{
		streak = self.cur_kill_streak;
		
		if ( streak < level.ex_wmd_uav )
		return;
		
		if ( streak == level.ex_wmd_uav || streak == level.ex_wmd_uav2 )
		{
			self giveHardpoint( "radar_mp", streak );
		}
		else if ( streak == level.ex_wmd_sentrygun || streak == level.ex_wmd_sentrygun2 )
		{
			self giveHardpoint( "sentrygun_mp", streak );
		}
		else if ( streak == level.ex_wmd_airstrike || streak == level.ex_wmd_airstrike2 )
		{
			self giveHardpoint( "airstrike_mp", streak );
		}
		else if (( streak == level.ex_wmd_chopper || streak == level.ex_wmd_chopper2) && (isDefined( level.heli_paths ) || level.heli_paths.size) )
		{
			self giveHardpoint( "helicopter_mp", streak );
		}
		else if ( streak == level.ex_wmd_napalm || streak == level.ex_wmd_napalm2 )
		{
			self giveHardpoint( "napalm_mp", streak );
		}
		else if ( streak == level.ex_wmd_stealth || streak == level.ex_wmd_stealth2 )
		{
			self giveHardpoint( "stealth_mp", streak );
		}
		else if ( streak == level.ex_wmd_ac130 || streak == level.ex_wmd_ac1302 )
		{
			self giveHardpoint( "ac130_mp", streak );
		}
		else if ( streak == level.ex_wmd_nuke )
		{
			self giveHardpoint( "nuke_mp", streak );
		}	
		else if ( streak >= level.ex_kill_streaks )  
		{
			if ( (streak % 5) == 0 )
...
giveHardpoint( hardpointType, streak )
{
	...
	if( self thread giveHardpointItem( hardpointType ) )
...
triggerHardPoint( hardpointType )
{
	...
	if ( hardpointType == "radar_mp" )
	{
		...
		if(level.ex_drone_flyer)
		{
			self thread extreme\_ex_uav::uav_drone_go();
		}			
	}
	
	else if ( hardpointType == "sentrygun_mp" )
	{
		...
		if ( isDefined( level.chopper_manned ) )
		{
			self iPrintLnBold("Sentrygun not available");
			return false;
		}
		team = self.pers["team"];
		...
		self thread extreme\_ex_sentrygun::main();
	}
	...
	else if ( hardpointType == "helicopter_mp" )
	{
		...
		if(level.ex_helicopter_manned) thread maps\mp\_helicopter_manned::heli_think( self, startnode, self.pers["team"] );
		else if(level.ex_helicopter_unmanned) thread maps\mp\_helicopter_unmanned::heli_think( self, startnode, self.pers["team"] );
	...
	else if ( hardpointType == "stealth_mp" )
	{
		if ( isDefined( level.airstrikeInProgress ) )
		{
			self iPrintLnBold( level.hardpointHints[hardpointType+"_not_available"] );
		...
		result = self selectBomberLocation( hardpointtype );
	...
	else if ( hardpointType == "ac130_mp" )
	{
		...
		result = self thread extreme\_ex_ac130::ac130_attachPlayer_go();
	...
	else if ( hardpointType == "nuke_mp" )
	{
		...
		result = self thread extreme\_ex_nuke::main();
...
useRadarItem()
{
	...
	if ( level.teambased )
	{
		UAVAcquiredPrintAndSound( team, otherteam, self, level.ex_radarViewTime );
	...
	else
	{
		...
		self iprintln( &"MP_WAR_RADAR_ACQUIRED", self, level.ex_radarViewTime );
...
selectBomberLocation( hardpointtype )
{
	self beginLocationSelection( "map_artillery_selector", level.artilleryDangerMaxRadius * 1.2 );
	self.selectingLocation = true;
	...
	if ( isDefined( level.bomberInProgress ) )
	{
		self iPrintLnBold( level.hardpointHints["bomber_mp_not_available"] );
		self thread stopBomberLocationSelection( false );
		return false;
	}
	
	self.ex_hardpointtype = hardpointtype;
	self thread finishBomberUsage( location, ::useBomber );
	return true;
}

finishBomberUsage( location, usedCallback )
{
	self notify( "used" );
	wait ( 0.05 );
	self thread stopBomberLocationSelection( false );
...
endSelectionOn( waitfor )
{
	...
	self thread stopBomberLocationSelection( (waitfor == "disconnect") );
}
...
stopBomberLocationSelection( disconnected )
{
	if ( !disconnected )
	{
		self endLocationSelection();
...
useBomber( pos, hardpointtype )
{
	trace = bullettrace( self.origin + (0,0,10000), self.origin, false, undefined );
	pos = (pos[0], pos[1], trace["position"][2] - 514);

	thread extreme\_ex_stealthbomber::main( pos, hardpointtype, self, self.pers["team"] );
}
