...
init()
{
	...
	initPerkData( "specialty_twoprimaries" );
	initPerkData( "c4_mp" );
...
giveLoadout( team, class )
{
	...
	{
		self thread oldgiveLoadout( self.pers["team"], self.pers["class"] );
		return;
	}
	...
	else if( self.pers[class]["loadout_camo"] == "camo_gold" )
	...
	else if( self.pers[class]["loadout_camo"] == "camo_brockhuard" )
	...
	if ( primaryWeapon != "none" )
	{
		...
		if( self.pers[class]["loadout_camo"] == "camo_none" )
...
oldgiveLoadout( team, class )
{
	...
	*/
	...
	{	
		...
		self.rankclass = self.custom_class[class_num];
		...
		if( isDefined( grenadeTypePrimary ) )
		{
		...
		if( isDefined( grenadeTypeSecondary ) )
		{ 
...
{
	if( !isDefined( self.custom_class ) )
		return;
...
/*		iprintln( "\n ========== CLASS DEBUG INFO ========== \n" );
		iprintln( "Primary: "+self.custom_class[i]["primary"] );
		iprintln( "Secondary: "+self.custom_class[i]["secondary"] );
		iprintln( "Specialty1: "+self.custom_class[i]["specialty1"]+" - Group: "+self.custom_class[i]["specialty1_group"]+" - Count: "+self.custom_class[i]["specialty1_count"] );
		iprintln( "Specialty2: "+self.custom_class[i]["specialty2"] );
		iprintln( "Specialty3: "+self.custom_class[i]["specialty3"] );
		iprintln( "Special Grenade: "+self.custom_class[i]["special_grenade"]+" - Count: "+self.custom_class[i]["special_grenade_count"] );
		iprintln( "Primary Camo: "+attachmenttablelookup( level.cac_cname, level.cac_cint2, camo_num ) );*/
...
cac_init()
{
	...
	if( getDvar( "perk_allow_c4_mp" ) == "1" )
		level.allowedPerks[0][ 4] = perkReferenceToIndex[ "specialty_weapon_c4" ];
	else
		level.allowedPerks[0][ 4] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_specialgrenade" ) == "1" )
		level.allowedPerks[0][ 5] = perkReferenceToIndex[ "specialty_specialgrenade" ];
	else
		level.allowedPerks[0][ 5] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_rpg_mp" ) == "1" )
		level.allowedPerks[0][ 6] = perkReferenceToIndex[ "specialty_weapon_rpg" ];
	else
		level.allowedPerks[0][ 6] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_claymore_mp" ) == "1" )
		level.allowedPerks[0][ 7] = perkReferenceToIndex[ "specialty_weapon_claymore" ];
	else
		level.allowedPerks[0][ 7] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_fraggrenade" ) == "1" )
		level.allowedPerks[0][ 8] = perkReferenceToIndex[ "specialty_fraggrenade" ];
	else
		level.allowedPerks[0][ 8] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_extraammo" ) == "1" )
		level.allowedPerks[0][ 9] = perkReferenceToIndex[ "specialty_extraammo" ];
	else
		level.allowedPerks[0][ 9] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_detectexplosive" ) == "1" )
		level.allowedPerks[0][10] = perkReferenceToIndex[ "specialty_detectexplosive" ];
	else
		level.allowedPerks[0][10] = perkReferenceToIndex[ "specialty_null" ];
	
	level.allowedPerks[1][ 0] = 190;
	if( getDvar( "perk_allow_specialty_bulletdamage" ) == "1" )
		level.allowedPerks[1][ 1] = perkReferenceToIndex[ "specialty_bulletdamage" ];
	else
		level.allowedPerks[1][1] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_armorvest" ) == "1" )
		level.allowedPerks[1][ 2] = perkReferenceToIndex[ "specialty_armorvest" ];
	else
		level.allowedPerks[1][ 2] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_fastreload" ) == "1" )
		level.allowedPerks[1][ 3] = perkReferenceToIndex[ "specialty_fastreload" ];
	else
		level.allowedPerks[1][ 3] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_rof" ) == "1" )
		level.allowedPerks[1][ 4] = perkReferenceToIndex[ "specialty_rof" ];
	else
		level.allowedPerks[1][ 4] = perkReferenceToIndex[ "specialty_null" ];
//	if( getDvar( "perk_allow_c4" ) == "1" )
		level.allowedPerks[1][ 5] = perkReferenceToIndex[ "specialty_twoprimaries" ];
//	else
//		level.allowedPerks[1][ 5] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_gpsjammer" ) == "1" )
		level.allowedPerks[1][ 6] = perkReferenceToIndex[ "specialty_gpsjammer" ];
	else
		level.allowedPerks[1][ 6] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_explosivedamage" ) == "1" )
		level.allowedPerks[1][ 7] = perkReferenceToIndex[ "specialty_explosivedamage" ];
	else
		level.allowedPerks[1][ 7] = perkReferenceToIndex[ "specialty_null" ];
	
	level.allowedPerks[2][ 0] = 190;
	if( getDvar( "perk_allow_specialty_longersprint" ) == "1" )
		level.allowedPerks[2][ 1] = perkReferenceToIndex[ "specialty_longersprint" ];
	else
		level.allowedPerks[2][ 1] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_bulletaccuracy" ) == "1" )
		level.allowedPerks[2][ 2] = perkReferenceToIndex[ "specialty_bulletaccuracy" ];
	else
		level.allowedPerks[2][ 2] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_pistoldeath" ) == "1" )
		level.allowedPerks[2][ 3] = perkReferenceToIndex[ "specialty_pistoldeath" ];
	else
		level.allowedPerks[2][ 3] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_grenadepulldeath" ) == "1" )
		level.allowedPerks[2][ 4] = perkReferenceToIndex[ "specialty_grenadepulldeath" ];
	else
		level.allowedPerks[2][ 4] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_bulletpenetration" ) == "1" )
		level.allowedPerks[2][ 5] = perkReferenceToIndex[ "specialty_bulletpenetration" ];
	else
		level.allowedPerks[2][ 5] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_holdbreath" ) == "1" )
		level.allowedPerks[2][ 6] = perkReferenceToIndex[ "specialty_holdbreath" ];
	else
		level.allowedPerks[2][ 6] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_quieter" ) == "1" )
		level.allowedPerks[2][ 7] = perkReferenceToIndex[ "specialty_quieter" ];
	else
		level.allowedPerks[2][ 7] = perkReferenceToIndex[ "specialty_null" ];
	if( getDvar( "perk_allow_specialty_parabolic" ) == "1" )
		level.allowedPerks[2][ 8] = perkReferenceToIndex[ "specialty_parabolic" ];
	else
		level.allowedPerks[2][ 8] = perkReferenceToIndex[ "specialty_null" ];
}