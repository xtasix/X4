...
/*
	CTF by Tally
	Based on COD2 CTFb by Matthias - admiral mod.
	12/17/2007
*/
...
main()
{
	...
	maps\mp\gametypes\_globallogic::registerTimeLimitDvar( "ctfb", 2, 0, 1440 );
	maps\mp\gametypes\_globallogic::registerScoreLimitDvar( "ctfb", 500, 0, 5000 );
	maps\mp\gametypes\_globallogic::registerRoundLimitDvar( "ctfb", 1, 0, 99 );
	maps\mp\gametypes\_globallogic::registerNumLivesDvar( "ctfb", 0, 0, 99 );
	
	level.ex_heavyflag 		= extreme\_ex_utils::dvardef("scr_ctfb_heavyflag", 1, 0, 1, "int");
	level.ex_flag_returndelay 	= extreme\_ex_utils::dvardef("scr_ctfb_returnflag_delay", 60, 0, 200, "int");
	level.ex_pointsfor_flag 	= extreme\_ex_utils::dvardef("scr_ctfb_pointsfor_flag", 10, 0, 99, "int");
	...
	game["dialog"]["gametype"] = "Capture_the_Flag_back";
}
	...
	for(;;)
	{
		...
		if(isPlayer(other) && isAlive(other) && (other.pers["team"] != "spectator"))
		{
			...
			if(other.pers["team"] == self.team)
			{
				if(self.atbase)
				{
					if(isdefined(other.flag))
					{
						...
						logPrint("A;" + lpselfguid + ";" + lpselfnum + ";" + other.name + ";" + "ctfb_captured_flag" + "\n");
				...
				else // Returned flag
				{
					...
					if(!isdefined(other.enemyflagAttached))
					{
			...
			else if(other.pers["team"] != self.team) // Pick Up Flag
			{
				...
				if(!isdefined(other.ownflagAttached))
				{
					if(self.team == "axis")
				...
				lpselfnum = other getEntityNumber();
				lpselfguid = other getGuid();
				logPrint("A;" + lpselfguid + ";" + lpselfnum + ";" + other.name + ";" + "ctfb_pickedup_flag" + "\n");
			}
		}
		wait 0.05;
	}
...
checkBaseHomeOwnFlag(flag)
{
	...
	while(isDefined(flag))
	{
		...
		if(isDefined(flag) && (self.sessionstate == "playing") && (distance(flag.home_origin, self.origin) < 50))
		{
		//	if(!isdefined(self.ownflagAttached)) break;
			...
			friendlyAlias = "mp_capture_flag";
			self PlaySoundToPlayer( friendlyAlias, self );
			...
			self thread [[level.onXPEvent]]( "return" );
			givePlayerScore( "return", self );
			level notify( "update_allhud_score" );

			lpselfnum = self getEntityNumber();
			lpselfguid = self getGuid ();
			logPrint("A;" + lpselfguid + ";" + lpselfnum + ";" + self.name + ";" + "ctf_returned_flag" + "\n");
...
pickupOwnFlag(flag)
{
	...
	if(self.ownflag.team == "axis")
		self iprintln( &"EXTREME_TAKEN_BACK_AXIS", self.name );
	else 
		self iprintln( &"EXTREME_TAKEN_BACK_ALLIES", self.name );
...
dropOwnFlag()
{
	...
	if(isdefined(self.ownflag))
	{
		...
		if(self.ownflag.team == "axis")
			iprintln( &"EXTREME_DROPPED_AXIS" );
		else	
			iprintln( &"EXTREME_DROPPED_ALLIES" );
