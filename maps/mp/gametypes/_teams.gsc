...
updateTeamBalance()
{
	...
	wait (.15 * level.fps_multiplier);

	if ( level.teamBalance && (level.roundLimit > 1 || (!level.roundLimit && level.scoreLimit != 1)) )
	{
		if( isDefined( game["BalanceTeamsNextRound"] ) )
			if(level.ex_showteambalance_msg) iPrintLnbold( &"MP_AUTOBALANCE_NEXT_ROUND" );
	...
	else
	{
		for(;; )
		{
			if( level.teamBalance )
			{
				if( !getTeamBalance() )
				{
					if(level.ex_showteambalance_msg) iPrintLnBold( &"MP_AUTOBALANCE_SECONDS", 15 );
					wait (15.0 * level.fps_multiplier);

					if( !getTeamBalance() ) level balanceTeams();
				}

				wait (59.0 * level.fps_multiplier);
			}
...
balanceTeams()
{
	if(level.ex_showteambalance_msg) iPrintLnBold( game["strings"]["autobalance"] );
