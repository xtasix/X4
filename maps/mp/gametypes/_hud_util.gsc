...
{
	if (getDvarInt("sv_fps") == 30)
	{
		level.fps_multiplier = 1.51382;
		level.frame = .033;
	}
	else if (getDvarInt("sv_fps") == 25)
	{
	...
	if ( isDefined( rateOfChange ) && barWidth < self.width ) 
	{
		if ( rateOfChange > 0 )
		{
			...
			self.bar scaleOverTime( (((1 - barFrac) / rateOfChange) / level.fps_multiplier) , self.width, self.height );
		}
		else if ( rateOfChange < 0 )
		{
			...
			self.bar scaleOverTime( ((barFrac / (-1 * rateOfChange)) / level.fps_multiplier), 1, self.height );
		}
	}
