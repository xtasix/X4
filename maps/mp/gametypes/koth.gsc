...
onStartGameType()
{
	...
	maps\mp\gametypes\_rank::registerScoreInfo( "assault", level.ex_score_assault );
...
//		maps\mp\gametypes\_globallogic::leaderDialog( "move_to_new" );
		...
		if ( level.hqSpawnTime )
		{
			...
			wait (level.hqSpawnTime * level.fps_multiplier );
		...
		radioObject maps\mp\gametypes\_gameobjects::setUseTime( level.captureTime * level.fps_multiplier );
...
onRadioCapture( player )
{
	...
	if ( !level.kothMode )
		self maps\mp\gametypes\_gameobjects::setUseTime( level.destroyTime * level.fps_multiplier );
...
awardHQPoints( team )
{
	...
	while ( !level.gameEnded )
	{
		...
		wait (seconds * level.fps_multiplier );
	}
}
