...
init()
{
	...
	if(level.ex_rank_stock || level.ex_fullwarfare)
	{
...
SetupCallbacks()
{
	...
	// eXtreme callbackPlayerDamage override
...
default_onForfeit( team )
{
	...
	wait (10.0 * level.fps_multiplier);
...
{	
	...
	if ( level.prematchPeriodEnd > 0 )
	{
		...
		wait (level.prematchPeriodEnd * level.fps_multiplier);
	}
...
matchStartTimerSkip()
{
	...
	if(level.ex_toggleThermal)
	{
		visionSetNight("thermal", 0.05 );
	}
}
...
spawnPlayer()
{
	...
	else
	{
		...
		if(self.classiscustom)
			self maps\mp\gametypes\_class::oldgiveLoadout( self.team, self.class );
		else
			self maps\mp\gametypes\_class::giveLoadout( self.team, self.class );
	}
	...
	{
		if(level.ex_moveinprematch) self freezeControls( false );
	...
	if ( getdvar( "ex_showperkson" ) == "" )

	setdvar( "ex_showperkson", "1" );

	if ( !level.splitscreen && getdvarint( "ex_showperkson" ) == 1 && game["state"] != "postgame" )
	{
		perks = getPerks( self );
		self showPerk( 0, perks[0], -50 );
		self showPerk( 1, perks[1], -50 );
	...
	if(!isdefined(self.pers["knife_kill"]))
		self.pers["knife_kill"] = 0;
	if(!isdefined(self.pers["grenade_kill"]))
		self.pers["grenade_kill"] = 0;
	if(!isdefined(self.pers["headshot_kill"]))
		self.pers["headshot_kill"] = 0;
...
in_spawnSpectator( origin, angles )
{
	...
	self thread extreme\_ex_hud::cleanPlayerHud(); //X4
...
endGame( winner, endReasonText )
{
	...
	extreme\_ex_main::exEndGameCleanUp(); //X4
	...
	for ( index = 0; index < players.size; index++ )
	{
		...
		if(level.ex_rank_stock) player openMenu( game["menu_eog_unlock"] );
	}
...
roundEndWait( defaultDelay, matchBonus )
{
	...
	while ( !notifiesDone )
	{
		...
		wait ( 0.5 * level.fps_multiplier);
	}

	if ( !matchBonus )
	{
		wait ( defaultDelay * level.fps_multiplier );
		return;
	}

	wait ( (defaultDelay / 2) * level.fps_multiplier );
...
menuClass( response )
{
	...
	if ( self.sessionstate == "playing" )
	{
		...
		if ( level.inGracePeriod && !self.hasDoneCombat ) // used weapons check?
		{
			...
			self maps\mp\gametypes\_class::oldgiveLoadout( self.pers["team"], self.pers["class"] );
		}
		else if ( !level.splitScreen )
		{
...
gracePeriod()
{
	...
	wait ( level.gracePeriod * level.fps_multiplier);
...
TimeUntilSpawn( includeTeamkillDelay )
{
	...
	if ( self.hasSpawned )
	{
		...
		if ( level.hardcoreMode && !isDefined( result ) && !respawnDelay )
			if(level.ex_hc_delay) respawnDelay = 0;
				else respawnDelay = 10.0;
...
waitForTimeOrNotify( time, notifyname )
{
	...
	wait (time * level.fps_multiplier);
}
...
Callback_StartGameType()
{
	...
	if ( !isDefined( game["gamestarted"] ) )
	{
		...
		game["strings"]["change_model"] = &"MP_CHANGE_MODEL_NEXT_SPAWN"; 
		game["strings"]["change_model_default"] = &"MP_CHANGE_MODEL_DEFAULT";
		game["strings"]["no_change_model"] = &"MP_NO_CHANGE_MODEL";
	...
	thread maps\mp\gametypes\_oldschool::init(); //X4
	thread maps\mp\gametypes\_battlechatter_mp::init();
	thread maps\mp\gametypes\_hardpoints::init();
	thread maps\mp\gametypes\_redirect::init(); //X4
	thread extreme\_ex_bloodyscreen::init(); //X4
	thread extreme\_ex_main::urlredirect(); //X4
	...
	makeDvarServerInfo( "ui_allow_classchange", getDvar( "ui_allow_classchange" ) ); //X4
	makeDvarServerInfo( "ui_allow_teamchange", getDvar( "ui_allow_teamchange" ) ); //X4
...
Callback_PlayerConnect()
{
	...
	else
	{
		...
		{
			self thread [[level.spawnClient]]();	
			//self extreme\_ex_clientcontrol::exPrintJoinedTeam(); //Joker - Wrong place!
		}
		else
		{
	...
	if(level.ex_favorite) 
	{
		   self setClientDvar("ui_favoriteAddress", game["ex_favoriteaddress"]);
		   wait .05;
		   self setClientDvar("ui_ex_addfavorite", 1);
	}
	else
		self setClientDvar("ui_ex_addfavorite", 0);	
}
...
forceSpawn()
{
	...
	wait ( 60.0 * level.fps_multiplier);
...
Callback_PlayerDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	if(isDefined(level.headshot_fulldamage))
	{
		if ( sMeansOfDeath == "MOD_HEAD_SHOT" ) iDamage = 200;
	}
...
//			}
		...
		if(level.ex_sticky_nades)	
		{
			...
			{ 
				...
				self playlocalSound("c4_bounce_default");
				self iprintlnbold("^1S T U C K!");
...
PostPlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime)
{
	...
	if(isPlayer(eAttacker) && distance(eAttacker.origin , self.origin ) < 150 )
		eAttacker thread extreme\_ex_bloodyscreen::Splatter_View();
}
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	self thread extreme\_ex_main::exPlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration);
	...
	if ( level.ex_bodysearch ) thread extreme\_ex_body::BodySearch( self, body );
	
	//Bodysink on Death
	body thread Bodysink();
	
	//bloodpool
	body thread bloodpool();
	...
	wait ( 0.25 * level.fps_multiplier);
...
getPerks( player )
{
	...
	if ( isPlayer( player ) && !level.oldschool )
	{
		...
		if ( level.onlineGame && isdefined(player.custom_class) )
		{
...
Bodysink()
{
	if (level.ex_bodysink)
	{ 
		wait level.ex_bodysink_time;
...
bloodpool()
{
	if (level.ex_bloodpools)
	{ 
		{
			wait 4; 
			playfx(level.ex_effect["bloodpool"], self.origin);
		}
	}
}
...
{ 
   ...
   wait(1); // add a 3 second wait to make it seem there realy is a nade attached. 
