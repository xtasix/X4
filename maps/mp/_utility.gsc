...
// Start - (c) 2007 by finalStats -  Free for non-commercial use

fSLog ( fSLogLine )
{
	
	if( getDvar( "fSConnect" ) == "1" )
	{
		

		while( game[ "fSLock" ] == 1 )
		{
			//logprint("Debug;Log;1\n");
			wait .05;
		}
		game[ "fSLock" ] = 1;

		if( game[ "fSPointer" ] < 32 )
		{

			game[ "fSLog_" + ( game[ "fSPointer" ] ) ] = fSLogLine;
			game[ "fSPointer" ] = game[ "fSPointer" ] + 1;
		...
		else
		{
			...
			for ( fSCleaner = 0; fSCleaner < game[ "fSPointer" ]; fSCleaner++ )
				game[ "fSLog_" + ( fSCleaner ) ] = undefined; 
...
fSQueue()
{
	...
	if( game[ "fSQueue" ] == true )
		return;

	game[ "fSQueue" ] = true;

	while( game[ "fSQueue" ] == true )
	{
		
		if( getDvar( "fSLog" ) == "" && game[ "fSPointer" ] > 0 )
		{
			...
			for ( fSBuilder = 0; fSBuilder < min( 4, game[ "fSPointer" ] ); fSBuilder++ )
				fSLog = fSLog + game[ "fSLog_" + ( fsBuilder ) ]; 

			for ( fSMover = fSBuilder; fSMover < game[ "fSPointer" ]; fSMover++ )
				game[ "fSLog_" + ( fSMover - fSBuilder ) ] = game[ "fSLog_" + ( fSMover ) ];

			for ( fSCleaner = game[ "fSPointer" ] - fSBuilder; fSCleaner < game[ "fSPointer" ]; fSCleaner++ )
				game[ "fSLog_" + ( fSCleaner ) ] = undefined;

			game[ "fSPointer" ] = game[ "fSPointer" ] - fSBuilder;

			//logprint("Debug;Queue;2\n");
			setDvar( "fSLog", fSLog );
			//logprint("Debug;Queue;3\n");
...
// End - (c) 2007 by finalStats -  Free for non-commercial use
set_vision_set( visionset )
{
	visionSetNight( visionset );
}