	...
	if(level.ex_amc_chutein)
	{
		...
		while(level.ex_amc_perteam)
		{
			...
			for(i = 0; i < drop_count; i++)
			{
				...
				wait(1); // A longer wait will increase mutual distance between planes within the same slice
			}
...
ammocratesOnGroundMonitor()
{
	...
	wait(0.1); // Wait in case all crates already touched ground (the monitor would fire its notify before the waittill started).
	if(level.ammocrate_debug) logprint("AMMOCRATES: waiting for " + getAmmocratesAllocated() + " crates to touch ground.\n");
	while(getAmmocratesAllocated() != getAmmocratesWithStatus("onground")) wait(1);
...
ammoCratePlane(ammocrate_index, plane_angle)
{
	...
	plane_models[0] = "vehicle_mi24p_hind_desert";
	plane_models[1] = "vehicle_cobra_helicopter_fly";
	...
	plane_sounds[0] = "mp_hind_helicopter"; 
	plane_sounds[1] = "mp_cobra_helicopter";
	...
	targetpos_z = level.ex_mapArea_Max[2] - randomint(150);
	if(level.ex_planes_altitude && (level.ex_planes_altitude <= targetpos_z))
		targetpos_z = level.ex_planes_altitude;
	...
	plane.angles = plane.angles + (00, plane_angle, 00);
	if(plane_index == 1) plane rotateyaw(10, .1);

	plane.sound = spawn("script_model", plane_startpos + (00, -50, 00));
	plane.sound linkto(plane);
	plane.sound playloopsound(plane_sound);
	
	if(plane_index == 0) plane spawnMi28Fx(); 
	if(plane_index == 1) plane spawncobraFx(); 
	...
	{
		if((distance(plane_droppos, plane.origin) < 100) && !crate_dropped)
		{
...
ammoCrateDrop(ammocrate_index)
{
	...
	crate_models[0] = "com_plasticcase_green_big";
	crate_models[1] = "com_plasticcase_black_big";
	...
	trace = bulletTrace(crate_endpos + (0, 0, 100), crate_endpos + (0, 0, -1200), false, undefined); //-1200  //100
	if(trace["fraction"] < 1.0)
	{
		ground = trace["position"];
		if(ground[2] > targetpos[2] && (ground[2] - targetpos[2] > 100)) ground = targetpos; //50
	}
	else ground = targetpos;

	// Create parachute
	crate.parachute = spawn("script_model", crate.origin);
	crate.parachute setModel("shaneys_chute_i"); 
	crate.parachute.angles = crate.angles + (0, 0, 0);
	...
	chute setModel("shaneys_chute_i"); //need model
	chute.angles = crate.parachute.angles;
	crate unlink();
	crate.parachute delete();
	crate.anchor delete();
	crate.origin = ground;
	crate playSound( "physics_car_door_default" ); //crate impact sound
...
ammoCrateThink()
{
	...
	while(!self.timeout)
	{
		...
		for(i = 0; i < players.size; i++)
		{
			...
			if(isPlayer(players[i]) && isDefined(players[i].ex_amc_msg) && !isDefined(players[i].ex_amc_rearm) && !isDefined(players[i].ex_amc_check))
			...
			//if(isPlayer(players[i]) && isDefined(players[i].ex_sprinting) && players[i].ex_sprinting || players[i] [[level.ex_getstance]](false) == 2) continue;

			// If player is ADS, do not rearm
			if(isPlayer(players[i]) && players[i] playerADS()) continue;

			if(isPlayer(players[i]) && players[i].sessionstate == "playing" && players[i] isOnGround() && !isDefined(players[i].ex_amc_rearm))
			{
	...
	wait(0.1); // Wait for all threads to die
...
ammoCratePlayerRearm(crate)
{
	...
	while(monitor && isDefined(crate) && !crate.timeout && isPlayer(self) && self.sessionstate == "playing" && distance(self.origin, crate.origin) < 36 ) //&& self [[level.ex_getstance]](true) != 2)
	{
		...
		count = 0;
		while(self useButtonPressed() && count < 10)
		{
			wait 0.05;
			count++;
		}
		...
		if(self useButtonPressed())
		{
			...
			if(isDefined(crate)) self linkTo(crate);
			...
			while(isPlayer(self) && isDefined(crate) && self useButtonPressed() && progresstime <= prog_limit && !crate.timeout)
			{
				...
				{
					self thread maps\mp\gametypes\_weapons::giveAmmoToPlayer();
					//hardpoints upgrade in crates
					self thread giveHPsupgrade(); 
					self thread ammoCrateMessage(&"AMMOCRATE_REARMING_GRENADES");
					weaponsdone = true;
				}
				else if(progresstime == 5 && !isDefined(grenadesdone))
				{
					self thread ammoCrateMessage(&"AMMOCRATE_REARMING_FIRSTAID");
					self thread replenishFirstaid();
					self playLocalSound("mp_bomb_defuse");
					grenadesdone = true;
				}
				else if(progresstime == 8 && !isDefined(item_health))
				{	
					item_health = true;
				}
			}
...
ammoCrateShowObjective()
{
	...
	objective_add(crate_objnum, "current", self.origin, "compass_objpoint_ammo");
...
replenishFirstaid()
{
	...
	self playlocalsound("health_pickup");
...
giveHPsupgrade()
{
	...
	if(level.ex_amc_hardpoints)
	{
		
		if(randomInt(100) < 25) 
		{
			maps\mp\gametypes\_hardpoints::upgradeHardpointItem();
		}
	}
}