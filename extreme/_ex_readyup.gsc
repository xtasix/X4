init()
{
	...
	if(!isDefined(game["readyup_done"]))
	{
		...
		else if(level.ex_readyup == 2) // enhanced mode
		{
...
onPlayerConnect()
{
	...
	for(;;)
	{
		...
		player thread onPlayerDisconnect(lpselfnum);
	}
}
...
levelReadyup()
{
	...
	level.ruhud_status1.fontScale = 1.4;
	level.ruhud_status1.color = (1,1,1);
	...
	level.ruhud_status3.fontScale = 1.4;
...
playerReadyup(lpselfnum)
{
	...
	*/
	...
	self.ruhud_status.y = 330;
	self.ruhud_status.fontScale = 1.4;
	...
	self.ruhud_howto.y = 360;
	self.ruhud_howto.fontScale = 1.4;
	...
	while(level.ex_readyup_status == 2)
	{
		if(isPlayer(self) && self useButtonPressed())
		{
			...
			while(isPlayer(self) && self useButtonPressed()) wait .05;
		}
		else wait .05;
	}
